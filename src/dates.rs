...
pub fn generate_start_timestamp(days_to_commit: i64) -> OffsetDateTime {
    let now = OffsetDateTime::now();
    let corrected_now = now - (now.time() - START);
    corrected_now - Duration::days(days_to_commit)
}
